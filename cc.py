...
class C:
    def __init__(self):
        ...
        self.certificate = self.cc.get_certificate('AUTHENTICATION')
        ...
        self.chain = []
        self.send_chain = []
        for cert in self.cc.get_chain(subject):
            self.chain.append(cert)
            self.send_chain.append(b64encode(cert).decode('utf-8'))

    def do_thing(self):
        msg = {
            "cenas" : "aaaaaaaa123",
            "certificate" : b64encode(self.certificate).decode('utf-8'), 
            "chain" : self.chain
        }
        ...
        hasher = hashes.Hash(self.chosen_hash, default_backend())
        hasher.update(msg['cenas'].encode()),
